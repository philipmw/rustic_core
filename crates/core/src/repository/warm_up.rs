...
use crate::{
    ...
    repository::{Repository, uses_multiple_placeholders},
};
...
fn warm_up_command<P: ProgressBars>(
    ...
    let use_multiple = uses_multiple_placeholders(command)?;
    ...
    for batch in &chunks {
        ...
        if use_multiple {
            warm_up_batch_multiple(&batch, command, ty, backend, &p)?;
        } else {
            warm_up_batch_single(&batch, command, ty, backend, &p)?;
        }
    }
