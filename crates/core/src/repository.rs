...
/// Check if the command uses multiple placeholders (%ids or %paths)
/// Returns true if the command should be executed once with all values,
/// false if it should be executed once per pack
pub(super) fn uses_multiple_placeholders(command: &CommandInput) -> RusticResult<bool> {
...
impl<P> Repository<P, ()> {
    ...
    pub fn new_with_progress(
        ...
        if let Some(warm_up) = &opts.warm_up_command {
            let _ = uses_multiple_placeholders(warm_up)?;
